...
.EXAMPLE
  ...
  # in search: .xls, .xlsx, .xlsm
  .\wias.ps1 -Excel

  # Fult - normal operation with APIs / Keys / Tokens
  ## This ce false positives ## 
  .\wis.ps1 -FullCheck 

  # Addme stamps to each command
  .\weas.ps1 -TimeStamp
...
$Drives.Root | ForEach-Object {
  ...
  Get-ChildItem $Drive -Recurse -Include $fileExtensions -ErrorAction SilentlyContinue -Force | ForEach-Object {
    ...
    if ($Path.FullName | select-string "(?i).*(lang|ermolaev).*") {
